      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Manage promotional codes, discounts, and tracking.
          </p>
        </div>
        <button
        ...
        <div className="flex w-full border rounded-lg overflow-hidden sm:w-auto">
          {statusTabs.map((tab, i) => (
            <button
              ...
              className={`flex-1 px-5 py-2 text-xs font-semibold tracking-wider transition-colors sm:flex-none ${activeTab === i ? "bg-card text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"}`}
            >
              {tab}
            </button>
          ))}
              ...
              return (
                <div key={c.id} className="p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        <p className="font-bold font-mono text-foreground break-words">
                          {c.code}
                        </p>
                        <p className="text-xs text-muted-foreground">
                          {c.discount}{" "}
                          {c.discountType && (
                            <span className="ml-1">{c.discountType}</span>
                          )}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                      <span>
                        ...
                        <span className="text-xs text-muted-foreground ml-1">
                          / {c.max === Infinity ? "∞" : c.max}
                        </span>
                      </span>
                      <div className="h-1.5 w-20 bg-muted rounded-full overflow-hidden">
                        <div
                          className={`h-full rounded-full ${
                            isFull ? "bg-destructive" : "bg-primary/50"
                          }`}
                  ...
                  <div className="mt-3 flex items-center justify-end gap-1">
                    ...
                    <button
                      onClick={() =>
                        router.push(PAGES.ADMIN.COUPONS.EDIT("jskdlf"))
                      }
        ...
        <div className="overflow-x-auto">
          <table className="hidden w-full text-sm md:table">
            ...
            <tbody>
              {filteredCoupons.map((c) => {
                const usagePct =
                  c.max === Infinity ? 10 : (c.used / c.max) * 100;
                const isFull = c.used >= c.max && c.max !== Infinity;
                return (
                  <tr
                    key={c.id}
                  ...
                  >
                    ...
                    <td className="py-4 px-4 text-center">
                      <div className="flex flex-col items-center gap-1.5">
                        <span>
                          <span
                            className={`font-bold ${isFull ? "text-destructive" : "text-foreground"}`}
                          >
                            {c.used}
                          </span>
                          <span className="text-xs text-muted-foreground ml-1">
