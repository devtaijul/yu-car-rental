...
const roles = [
    ...
    },
    {
        name: "Support Staff",
        description:
            "Read-only access to bookings and users for support purposes.",
        users: 15,
    },
];
                ...
                <tbody>
                    {roles.map((role) => (
                        <tr key={role.name} className="border-b last:border-0">
                            <td className="py-3">
                <span className="px-3 py-1 rounded-md bg-foreground text-card text-xs font-semibold whitespace-nowrap">
                  {role.name}
                </span>
                            </td>
